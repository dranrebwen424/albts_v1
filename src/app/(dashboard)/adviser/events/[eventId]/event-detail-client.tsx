...
export function AdviserEventDetailClient({
  ...
  if (isMobile) {
    ...
    const formsNeedsReview = forms.filter(f => f.status === 'pending' && !f.rejection_reason);
    ...
    <div className="bg-[#f8f8f8] min-h-screen">
      ...
      <div className="px-4">
        <AnimatePresence mode="wait">
          {expensesTab === 'expenses' ? (
            ...
            >
              {receipts.length === 0 && forms.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16">
                  ...
                  <p className="mt-4 text-[13px] text-[#afafaf] text-center max-w-[220px] leading-relaxed">
                    No expenses yet. They will appear here once submitted.
                  </p>
                </div>
              ) : (
                <div className="space-y-6">
                  ...
                  {/* Needs Review */}
                  {formsNeedsReview.length > 0 && (
                    <div>
                      <h3 className="text-[13px] font-medium text-[#6b6b6b] mb-2">Needs Review</h3>
                      <div className="space-y-2">
                        {formsNeedsReview.map((form, index) => (
                          <motion.div
                            key={form.id}
                            initial={{ opacity: 0, y: 12 }}
                            animate={{ opacity: 1, y: 0 }}
                          ...
                          >
                            <div className="bg-[#ffffff] rounded-2xl p-3.5 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
                              <button onClick={() => setSelectedForm(form)} className="text-left w-full block">
                                <div className="flex items-start gap-3">
                                  <div className="h-[52px] w-[52px] rounded-[10px] bg-amber-50 flex items-center justify-center shrink-0">
                                    <FileText className="h-5 w-5 text-amber-600" />
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <p className="text-[13px] font-medium text-[#000000] truncate">{form.expense_name || form.expense_type}</p>
                                    <p className="text-[11px] text-[#6b6b6b] mt-0.5 capitalize truncate">{form.expense_type}</p>
                                    <p className="text-[11px] text-[#6b6b6b] mt-0.5 line-clamp-2">{form.description}</p>
                                    <div className="flex items-center justify-between mt-1">
                                      <span className="text-[13px] font-semibold text-[#000000]">{formatCurrency(form.amount)}</span>
                                      <span className="text-[10px] text-[#afafaf]">{formatDate(form.created_at)}</span>
                                    </div>
                                    {form.profiles && (
                                      <p className="text-[10px] text-[#afafaf] mt-0.5 truncate">{form.profiles.first_name} {form.profiles.last_name}</p>
                                    )}
                                  </div>
                                </div>
                              </button>
                              <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
                                <button
                                  ...
                                  className="flex-1 py-2 bg-[#01a550] text-white rounded-xl text-[12px] font-medium disabled:opacity-50"
                                >
                                  {approvingForm ? 'Approving...' : 'Approve'}
                                </button>
                                <button
                                  onClick={(e) => { e.stopPropagation(); setRejectTarget(form.id); }}
                                  disabled={rejectingForm || approvingForm}
                                  className="flex-1 py-2 bg-[#ef4444] text-white rounded-xl text-[12px] font-medium disabled:opacity-50"
                                >
                                  Reject
                                </button>
                              </div>
                  ...
                  {/* Recent */}
                  {(() => {
                    const recentItems = [...forms]
                      .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
                      .slice(0, 2);
                    return recentItems.length > 0 ? (
                      <div>
                        <h3 className="text-[13px] font-medium text-[#6b6b6b] mb-2">Recent</h3>
                        <div className="space-y-2">
                          {recentItems.map((form, index) => (
                            <motion.div
                              key={`recent-form-${form.id}`}
                              initial={{ opacity: 0, y: 12 }}
                              animate={{ opacity: 1, y: 0 }}
                              transition={{ delay: index * 0.06, duration: 0.3, ease: 'easeOut' }}
                            >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-start justify-between gap-2">
                                        ...
                                        <span className="shrink-0 text-[11px] font-medium capitalize" style={{
                                          color: form.status === 'approved' ? '#01a550' : form.status === 'pending' ? '#f59e0b' : '#ef4444'
                                        }}>
                                          {form.status}
                                        </span>
                                      </div>
                  ...
                  {/* All Expenses */}
                  <div>
                    <h3 className="text-[13px] font-medium text-[#6b6b6b] mb-2">All Expenses</h3>
                    {mergedExpenses.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-8">
                        <ClipboardText className="h-10 w-10 text-[#d0d0d0]" weight="light" />
                        <p className="mt-3 text-[12px] text-[#afafaf]">No expenses match your filter.</p>
                      </div>
                    ) : (
                      <div className="space-y-3">
          ...
          <div className="space-y-4 min-w-0">
            ...
            {selectedForm.status === 'pending' && (
              <div className="flex gap-2 pt-2">
                <Button onClick={() => handleApproveForm(selectedForm.id)} disabled={approvingForm || rejectingForm} className="flex-1 bg-[#01a550] text-white rounded-xl h-10 text-[13px] font-medium">
                  {approvingForm ? 'Approving...' : 'Approve'}
                </Button>
                <Button onClick={() => { setRejectTarget(selectedForm.id); setSelectedForm(null); }} disabled={rejectingForm || approvingForm} className="flex-1 bg-[#ef4444] text-white rounded-xl h-10 text-[13px] font-medium">
                  Reject
                </Button>
              </div>
            )}
      ...
      {/* Reject Dialog */}
      <ResponsiveDialog open={!!rejectTarget} onOpenChange={(open) => { if (!open) { setRejectTarget(null); setRejectReason(''); } }} title="Reject Form" description="Provide a reason for rejection">
        <div className="space-y-4">
          <div className="space-y-2">
      ...
      {/* Receipt Detail Modal (read-only) */}
