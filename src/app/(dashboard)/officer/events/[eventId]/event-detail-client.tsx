            ...
            <Card className="bg-surface-white rounded-2xl shadow-soft border border-divider/20 overflow-hidden">
              <CardContent className="p-6">
                <div className="flex flex-col items-center">
                  <BudgetChart used={totalExpenses} remaining={Math.max((event?.original_budget || 0) - totalExpenses, 0)} size={160} />
                  <div className="w-full mt-5 grid grid-cols-3 divide-x divide-divider/60">
                    <div className="text-center pr-2">
                      ...
                      <AnimatedCurrency value={event?.original_budget || 0} className="text-[14px] font-semibold text-text-primary leading-tight block mt-0.5" />
                    </div>
                    ...
                    <div className="text-center pl-2">
                      ...
                      <AnimatedCurrency value={(event?.original_budget || 0) - totalExpenses} className={cn('text-[14px] font-semibold leading-tight block mt-0.5', ((event?.original_budget || 0) - totalExpenses) < 0 ? 'text-error' : 'text-text-primary')} />
                    </div>
                  </div>
                  {((event?.original_budget || 0) - totalExpenses) < 0 && (
                    <div className="mt-3 w-full px-3 py-1.5 bg-red-50 rounded-xl text-[10px] font-medium text-error text-center">
                      Budget exceeded by {formatCurrency(Math.abs((event?.original_budget || 0) - totalExpenses))}
                    </div>
                  )}
                </div>
              </CardContent>
